...
export function printApiErrorMsgs(divId: string, error: any) {
    ...
    Object.keys(error).forEach((key: string) => {
        ...
        // Check if there are input elements with [name="key"]
        elements = div.querySelectorAll(`.validation input[name="${key}"]`);
        if (elements.length > 0) {
            const inputElement = elements[0] as HTMLInputElement;
            printErrMsg(inputElement, error[key][error[key].length - 1]);
            return; // Exit early since we found and processed an input element
        }

        // Check if there are divs with [data-error="key"]
        elements = div.querySelectorAll(`[data-error="${key}"]`);
        if (elements.length > 0) {
            const validationDiv = elements[0] as HTMLDivElement;
            printErrMsg(validationDiv, error[key][error[key].length - 1]);
            return; // Exit early since we found and processed a validation div
        }

        // Check if there are divs with [data-error="key"]
        elements = div.querySelectorAll(`[validation-error="${key}"]`);
        if (elements.length > 0) {
            const validationDiv = elements[0] as HTMLDivElement;
            printErrMsg(validationDiv, error[key][error[key].length - 1]);
            return; // Exit early since we found and processed a validation div
        ...
        // If no specific element found, try to find an input element within the div
        elements = div.querySelectorAll('.validation input[name]');
        if (elements.length > 0) {
            const inputElement = elements[0] as HTMLInputElement;
            printErrMsg(inputElement, error[key][error[key].length - 1]);
            return; // Exit early since we found and processed an input element within a validation div
        }
    });
}

export function printErrMsg(element: HTMLElement, message: string) {
    // Check if element is an input or a div
    if (element.tagName.toLowerCase() === 'input') {
        ...
        // Assume it's a div with class 'validation'
        element.textContent = message;
        element.style.color = '#dc3545';
        element.style.fontSize = '.875em';
    }
}
...
export function clearErrMsgs(divId: string) {
    ...
    const inputs = div.querySelectorAll('.is-invalid');
    ...
    const validationDivs = div.querySelectorAll('.validation');
    validationDivs.forEach(validationDiv => {
